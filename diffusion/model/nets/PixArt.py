...
class PixArt(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        assert "glyph" in kwargs, "Please provide the glyph"
        glyph = kwargs["glyph"]
        assert "glyph_mask" in kwargs, "Please provide the glyph_mask"
        glyph_mask = kwargs["glyph_mask"]
        for block in self.blocks:
            x = auto_grad_checkpoint(block, x, y, t0, glyph, mask, glyph_mask)  # (N, T, D) #support grad checkpoint
