...
def build_optimizer_glyph(model, byt5_encoder, byt5_mapper, optimizer_cfg, **byt5_kwargs):
    # default parameter-wise config
    logger = get_root_logger()
    ...
    # 增加byt5_encoder和byt5_mapper的参数
    if byt5_encoder is not None:
        optimizer.add_param_group({'params': byt5_encoder.parameters(), 
                'lr': optimizer_cfg['lr'] * byt5_kwargs['byt5_lr_scale'],
                'weight_decay':byt5_kwargs['byt5_weight-decay'],})
    optimizer.add_param_group({'params': byt5_mapper.parameters()})
    weight_decay_groups = dict()
    lr_groups = dict()
    for group in optimizer.param_groups:
        if not group.get('requires_grad', True): continue
